...
@interface ANPlayerView ()<ANPlayerControlSliderDelegate, ANPlayerPlayPausedViewDelegate>
{
    NSURL *_url;
    NSTimer *_timer;
}
...
//全屏播放器
@property (nonatomic,strong) ANPlayerView *fullScreenPlayer;
...
-(void)initAsset{
    ...
    if (_url) self.assert = [[AVURLAsset alloc]initWithURL:_url options:nil];
...
-(void)addPlayerLoading{
    ...
    self.loadingView = [[ANPlayerLoadingView alloc]init];
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        switch (status) {
            case AVPlayerStatusUnknown:{
                NSLog(@"未知状态");
                self.state = ANPlayerStateBuffering;
            }
                break;
            case AVPlayerStatusReadyToPlay:{
                NSLog(@"开始播放状态");
                self.state = ANPlayerStatePlaying;
                //总时长
            ...
            case AVPlayerStatusFailed:
                self.state = ANPlayerStateFailed;
        ...
        //全屏或者小屏
        if (kAN_SCREEN_WIDTH < kAN_SCREEN_HEIGHT) {
...
-(ANPlayerPlayPausedView *)createPlayPausedView{
    ANPlayerPlayPausedView *tmpView=[[ANPlayerPlayPausedView alloc]init];
...
-(void)addPlayerControl{
    self.playerControl=[[ANPlayerControl alloc]init];
...
//设置视频填充模式
-(void)setContentMode:(ANPlayerContentMode)contentMode{
    switch (contentMode) {
        case ANPlayerContentModeResizeFit:
            self.playerLayer.videoGravity=AVLayerVideoGravityResizeAspect;
            break;
        case ANPlayerContentModeResizeFitFill:
            self.playerLayer.videoGravity=AVLayerVideoGravityResizeAspectFill;
            break;
        case ANPlayerContentModeResize:
...
-(UIViewController *)pushToFullScreen{
    ...
    self.fullScreenPlayer=[[ANPlayerView alloc]initWithURLAsset:self.assert];
